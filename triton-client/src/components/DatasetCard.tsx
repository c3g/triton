import { Button, Modal, Popover, Space, Spin, Typography } from 'antd'
...
import { ActionDropdown, StagingAction } from './ActionDropdown'
...
function DatasetCard({ datasetID }: DatasetCardProps) {
	...
	const project = useAppSelector((state) =>
		dataset?.external_project_id ? state.projectsState.projectsById[dataset.external_project_id] : undefined
	)
		...
		return SUPPORTED_DOWNLOAD_TYPES.map((type) => {
			...
			if (req && !req.should_delete) {
				...
				const actions: StagingAction[] = [
					{
						action: { name: 'Unstage dataset', actionCall: () => deleteRequest() },
				...
				if (status === 'SUCCESS') {
					...
					popupDescription = 'Dataset is available for download. You should have received an email with download instructions.'
				} else if (status === 'FAILED') {
					statusDescription = 'FAILED'
					popupDescription = 'Dataset failed to stage. The IT team has been notified.'
				} else {
					statusDescription = 'QUEUED'
					popupDescription = 'Dataset is queued for staging. You will be notified by email when it is ready.'
				}
				const buttonStagingActive = (
					<Button key={type} style={{ paddingLeft: '4', paddingRight: '4' }} disabled={updatingRequest}>
						<Space>
							{type}
							{'|'}
							{statusDescription}
				...
				return (
					<Popover key={type} content={popupDescription} trigger="hover">
						<ActionDropdown button={buttonStagingActive} actions={actions} disabled={statusDescription !== 'SUCCESS'} />
					</Popover>
				)
			} else {
				return (
					<Button
						...
						disabled={!totalSize || alreadyRequested || updatingRequest || !dataset || !project}
		...
		>
			<div style={{ display: 'flex', justifyContent: 'space-between' }}>
				...
				<Space>
					...
					<div>{!totalSize ? <Spin /> : <DataSize size={totalSize} />}</div>
				</Space>
			</div>
		</div>
	) : (
